            ...
            try
            {
                ...
                foreach (var organization in organizations)
                {
                    if (organization.name.ToString().ToLower().Trim() == name.ToLower().Trim() && !domainFound)
                    {
                        ...
                        var requestGet = $"https://api.apollo.io/v1/organizations/enrich?api_key={_apiKey}&domain={domain}";
                        ...
                        try
                        {
                            numeployess = _dynamicResult.organization.estimated_num_employees?.ToString() ?? null;
                        }
                        catch (Exception)
                        {
                        ...
                        try
                        {
                            revenue = _dynamicResult.organization.annual_revenue?.ToString() ?? null;
                        }
                        catch (Exception)
                        {
                        ...
                        try
                        {
                            industry = dynamicResult.organization.industry?.ToString() ?? null;
                        }
                        catch (Exception)
                        {
                        }
                        ...
                        // if three fields are null, then return null
                        if (numeployess == null && revenue == null && industry == null)
                        {
                            return null;
                        }
